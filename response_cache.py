...
The cache does not understand experiments or model semantics.  A caller supplies a
...
class ResponseCache:
    ...
    def put(self, key: str, reply: dict) -> Path:
        ...
        path = self._path(key)
        if path.is_file():
            self.get(key)  # validate an entry written by an earlier call
            return path
        stored = {field: reply.get(field) for field in REPLY_FIELDS}
        artifact = {
