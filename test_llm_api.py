...
class SpecTests(unittest.TestCase):
    def test_resolve_reads_the_providers_secret(self):
        with mock.patch.dict(os.environ, {"NVIDIA_API_KEY": "nv-key"}):
    ...
    def test_errors_are_explicit(self):
        ...
        with mock.patch.dict(os.environ, {}, clear=True), mock.patch.dict("sys.modules", {"kaggle_secrets": None}):
...
class FromApiTests(unittest.TestCase):
    ...
    def test_reasoning_model_spec_overrides_the_arguments(self):
        client = FakeClient()
        spec = {"provider": "openai", "model": "gpt-5", "api_key": "sk", "token_param": "max_completion_tokens",
                "temperature": None}
        runner = dp.VisionRunner.from_api(spec, max_tokens=64, temperature=0.0, client=client)
    ...
    def test_adapter_from_spec(self):
        ...
        spec = {"provider": "nvidia", "model": "google/gemma-4-31b-it", "api_key": "nv-secret-123", "token_param": "max_tokens",
