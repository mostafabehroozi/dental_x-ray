...
class PathAndRoleTests(unittest.TestCase):
    ...
    def test_location_truth_is_shared_by_adapter_not_by_experiment(self):
        ...
        with patch.object(xp.llm_api, "connect", return_value=object()):
            adapter = xp.location_adapter(areas)
            self.assertEqual(adapter.kind, "areas")
            self.assertFalse(hasattr(adapter, "parser"))  # the areas are read by code, not by the parser service
