...
* AreaAdapter: the same kind of model, asked once per image about the untouched radiograph -
  not about the findings. It returns where the four quadrants lie in THIS image as normalized
  areas, because that is what patient positioning, arch shape, centring and missing teeth move;
  Python then places every ground-truth box in the area covering most of it (or, when no area
  touches it, the nearest one). The model never sees a box, so it cannot classify a finding, and
  the placement is deterministic and auditable: the areas, the reply and a marked image are saved.
...
Every adapter ends in the quadrant vocabulary (UR, UL, LL, LR), the finest scheme the pipeline
names; the arch level derives from it.
...
# ----------------------------------------------------------------------------
# Area adapter: this image's own region areas, then deterministic geometry per box
# ----------------------------------------------------------------------------
# The region schema: one anatomical definition per region name, in window order. Swapping this
# dictionary (and REGION_WINDOWS with it) is all another named set of regions needs; the prompt,
# the validation and the placement geometry read the names from here.
REGION_DEFINITIONS = {
    "UR": "the patient's upper RIGHT quadrant: the maxillary teeth from the midline between the two upper "
          "central incisors back to behind the last upper molar on that side. It lies in the TOP-LEFT of the image.",
    "UL": "the patient's upper LEFT quadrant: the maxillary teeth from the same midline back to behind the last "
          "upper molar on the other side. It lies in the TOP-RIGHT of the image.",
    "LL": "the patient's lower LEFT quadrant: the mandibular teeth from the midline between the two lower central "
          "incisors back to behind the last lower molar on that side. It lies in the BOTTOM-RIGHT of the image.",
    "LR": "the patient's lower RIGHT quadrant: the mandibular teeth from the same midline back to behind the last "
          "lower molar on the other side. It lies in the BOTTOM-LEFT of the image.",
}
assert tuple(REGION_DEFINITIONS) == QUADRANTS, "every region window needs one definition to be asked about"
...
def area_prompt() -> str:
    ...
    shape = ", ".join('{"region": "%s", "area": [x1, y1, x2, y2]}' % name for name in REGION_DEFINITIONS)
    return AREA_PROMPT.format(region_lines=lines, shape=shape)


def parse_areas(text: str, regions: tuple[str, ...] = QUADRANTS) -> tuple[dict[str, list[float]], str | None]:
    """{region: [x1, y1, x2, y2]} in window order, or ({}, reason) when the set is not complete and valid.

    A reply is accepted only as a whole: every region once, four numbers each, inside [0, 1] and with a
...
def place_box(box: dict, areas: dict[str, list[float]]) -> dict:
    """The one region of a box: the area covering most of it, or, when none touches it, the nearest area.

    Equal scores fall to the first region in window order (max and min keep the first of equal values),
    so the same box always lands in the same region. The scores behind the decision are kept.
    """
...
class AreaAdapter:
    """This image's region areas from a vision API, then every box placed by geometry in Python.

    One call per image, on the untouched radiograph: the model never sees the ground-truth boxes, so it
    cannot classify a finding - it only says where this patient's quadrants lie, which is the part that
    ...
    placed separately and identical boxes always land in the same region. A reply that is not a complete
    valid set of areas is retried and then follows failure_policy; the areas, the raw reply and the
    marked image are saved for audit. from_api() and the request options are as in LLMAdapter.
    """
    ...
    def settings(self) -> dict:
        return {"kind": self.kind, "model": self.model, "base_url": self.base_url, "token_param": self.token_param,
                ...
                "parse_retries": self.parse_retries, "api_call_retries": self.api_call_retries,
                "failure_policy": self.failure_policy, "regions": list(QUADRANTS),
    ...
    def adapt(self, image_path: str | Path, boxes: list[dict], image_id: str | None = None,
              drawn_dir: str | Path | None = None) -> list[dict]:
        """One record per box: its region, the areas it was placed against and how it was placed."""
